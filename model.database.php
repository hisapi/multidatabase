...
class Database_Postgres extends Relational_SQL_Database
{
	...
	public function connect()
	{
		...
		try
		{
			$this->db= pg_connect("host=".$this->url." dbname=".$this->dbname." user=".$this->auth1." password=".$this->auth2) or $failed=true;
			if ($failed && $this->debug)
			{
...
abstract class Database_Adapter
{
	...
	function is_mysql()
	{
		if ( strpos($this->kind,"mysql")!==false )
		{
			return true;
		}
		else
	...
	function is_mongo()
	{
		if ( strpos($this->kind,"mongo")!==false )
		{
			return true;
		}
		else
	...
	function is_mssql()
	{
		if ( strpos($this->kind,"mssql")!==false || strpos($this->kind,"ms sql")!==false )
		{
			return true;
		}
		else
	...
	function is_oracle()
	{
		if ( strpos($this->kind,"oracle")!==false )
		{
			return true;
		}
		else
	...
	{
		if ( strpos($this->kind,"dynamodb")!==false )
		{
			return true;
		}
		else
